...
class Sqs extends Engine {
    ...
    listen(options) {
        return this.pull(options)
            .then((messages) => {
                if (messages.length) {
                    this.emit('message', _.map(messages, 'body'));
                }

                return Promise.map(messages, (msg) => {
                    return this.deleteMessage(msg.ref);
                });
            })
            .catch((err) => {
                this.emit('error', err);
            ...
            .finally(() => {
                process.nextTick(() => this.listen(options));
            });
    }
